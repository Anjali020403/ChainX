...
def load_dataset():
    ...
    with open(dataset_file, 'r') as file:
        ...
        latest_data = []  # List to store the data rows

        for row in csv_reader:
            data_row = {}
            for i, value in enumerate(row):
                # Assign default column names: Column1, Column2, ...
                column_name = f"Column{i + 1}"
                data_row[column_name] = value.strip()  # Create key-value pairs for each column
            latest_data.append(data_row)
